...
def test_enable_mode_comes_from_config_and_doctor_warns(config: RobotConfig, bare_config: RobotConfig, tmp_path):
    ...
    pulse_cfg = load_config(config_dir=tmp_path, use_env=False, layout="A", overrides=["safety.enable_mode=pulse"])
    assert check_interlock(pulse_cfg).status == PASS
    no_drive = load_config(config_dir=tmp_path, use_env=False, layout="A", overrides=["actuators.drivers.dc.type=none"])
    assert not has_drivetrain(no_drive) and check_interlock(no_drive).status == PASS
    # a fresh install (no hardware.yaml) has no drivetrain and must not be nagged about the interlock
    assert not has_drivetrain(bare_config) and check_interlock(bare_config).status == PASS
    gpio = MockGpioBackend()
